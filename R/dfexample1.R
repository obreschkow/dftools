...
dfexample1 <- function(n = 1e3, seed = 1, sigma = 0.5, p.true = c(-2,10,-1.3), include.mrp = FALSE) {
  ...
  # plot covariances
  dfplotcov(survey,reference=p.true,title='Schechter parameter covariances',model.col='blue')
  if (include.mrp) dfplotcov(survey2,reference=c(p.true,1),model.col='red',title='MRP parameter covariances')

  # plot main plot
  x = seq(2,12,0.01)
  nbins = max(4,round(sqrt(n)/2))
  mfplot(survey, xlim=c(1e6,2e11), ylim=c(1e-4,2), nbins = nbins, bin.xmin=7, bin.xmax=11)
